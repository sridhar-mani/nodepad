...
export function TileIndex({ blocks, onHighlight, highlightedId, onClose, isOpen, viewMode }: TileIndexProps) {
  ...
  return (
    <div 
      ...
      className={`fixed inset-y-0 right-0 z-drawer flex flex-col h-[100dvh] w-[min(92vw,20rem)] sm:w-[min(88vw,18rem)] bg-card/95 backdrop-blur-3xl border-l border-border shrink-0 overflow-hidden transition-all duration-200 ease-in-out lg:static lg:z-auto lg:h-full lg:w-auto lg:bg-card/85 ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}
    >
      <div className="w-full lg:w-[240px] flex flex-col h-full">
        ...
        <div className="flex h-10 items-center justify-between border-b border-border bg-card/70 backdrop-blur-md px-3 py-1.5 shrink-0">
          {onClose && (
            <button 
              onClick={onClose}
              className="p-1 px-1.5 hover:bg-secondary rounded-sm transition-colors text-muted-foreground/50 hover:text-foreground"
              title="Close Index"
            >
            ...
            <div className="space-y-1">
               ...
               {kanbanColumns.map(col => (
                 <button
                    ...
                    className="flex items-center gap-2.5 w-full px-2.5 py-2.5 rounded-sm transition-all hover:bg-secondary/60 text-left text-foreground/60 hover:text-foreground group"
                 >
              ...
              <div key={cat} className="space-y-1">
                ...
                {catBlocks.map(block => (
                   <button
                    ...
                    className={`flex items-start gap-2.5 w-full px-2.5 py-2 rounded-sm transition-all hover:bg-secondary/60 text-left group border-r-2 ${
